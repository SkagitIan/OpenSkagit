...
ID_PREFIX = "mc"
AMC_PATH_RE = re.compile(r"^/AMC/[A-Za-z0-9.\-]+$")
...
def search(jurisdiction: Dict[str, object], q: str, limit: int) -> List[Dict[str, object]]:
    ...
    search_url = f"{base_url}/search?q={quote(q)}&scope=all"
    headers = {
...
def get(jurisdiction: Dict[str, object], id_value: str) -> Dict[str, object]:
    ...
    doc_url = urljoin(base_origin, path)
    headers = {
        "User-Agent": USER_AGENT,
        "Accept": "text/html,application/xhtml+xml",
        "Referer": str(jurisdiction["base_url"]).rstrip("/") + "/",
    }
    try:
...
def _make_id(slug: str, path: str) -> str:
    payload = {"p": path}
...
def _parse_search_results(html: str, jurisdiction: Dict[str, object], limit: int) -> List[Dict[str, object]]:
    ...
    for anchor in soup.find_all("a", href=True):
        ...
        if not heading:
            continue
        context = normalize_inline_text(anchor.parent.get_text(" ", strip=True) if anchor.parent else "")
        snippet = context if context and context != heading else ""
        url = urljoin(base_origin, path)
        hits.append(
            {
                "id": _make_id(str(jurisdiction["slug"]), path),
                "cite": _derive_cite(path, heading),
                "heading": heading,
                "snippet": snippet,
                "url": url,
            }
...
def _parse_document(html: str, doc_url: str) -> Dict[str, object]:
    ...
    cite = _derive_cite(urlparse(doc_url).path, title)
    return {"cite": cite, "text": text, "url": doc_url}


def _extract_amc_path(href: str) -> Optional[str]:
    if not href:
        return None
    ...
    if not path.startswith("/"):
        return None
    if not AMC_PATH_RE.match(path):
        return None
    # Filter utility endpoints like /AMC/contents
    if path.lower() == "/amc/contents":
        return None
    return path


def _derive_cite(path: str, heading: str) -> Optional[str]:
    section_match = re.search(r"/AMC/([0-9]+(?:\.[0-9]+)+)$", path, flags=re.IGNORECASE)
    if section_match:
        return section_match.group(1)
    chapter_match = re.search(r"\bCh\.\s*([0-9]+(?:\.[0-9]+)+)\b", heading, flags=re.IGNORECASE)
    if chapter_match:
