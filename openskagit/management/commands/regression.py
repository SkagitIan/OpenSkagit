...
from openskagit.models import AdjustmentRunSummary  # adjust app label if needed
...
# --------------------------------------------------
# What Drives Value: groups of variables
# --------------------------------------------------
DRIVER_GROUPS = {
    "Size & layout": {
    ...
    "Lot & land": {
        "log_lot", "log_land_value", "land_share",
    },
    "Time & market cycle": {
        "t", "t_sq",
        "t_T1", "t_sq_T1",
        "t_T2", "t_sq_T2",
        "t_T3", "t_sq_T3",
        "area_time", "land_time", "value_time",
        "t_mv_T1", "t_sq_mv_T1",
        "t_mv_T2", "t_sq_mv_T2",
        "t_mv_T3", "t_sq_mv_T3",
    },
    "Location & extras": {
        "is_view", "has_garage", "has_basement",
    },
}
...
class Command(BaseCommand):
    ...
    def run_adjustment_model(self, df: pd.DataFrame, label: str, tier_name: str):
        ...
        # Use ratio_final as the "official" ratio going into diagnostics.
        ratio_use = df.get("ratio_final", df.get("ratio_adj", df["ratio"]))

        # ----------------------------------------------------------------------
        # 11. COD, PRD, PRB, PRB drivers (based on ratio_use)
        # ----------------------------------------------------------------------
        ...
        mid = len(df_sorted) // 2
        low_group = df_sorted.iloc[:mid]
        high_group = df_sorted.iloc[mid:]

        prd = high_group[ratio_use.name].mean() / low_group[ratio_use.name].mean()

        # PRB drivers
        prb_drivers = self.compute_prb_drivers(df.assign(ratio_iaao=ratio_use),
                                            final_predictors,
                                            ratio_col="ratio_iaao")

        # PRB slope on FINAL calibrated ratios
        V_proxy = df["V_proxy"]
        lo, hi = V_proxy.quantile([0.10, 0.90])
        mask = V_proxy.between(lo, hi)

        val_dev = ((V_proxy[mask] / V_proxy.median()) - 1.0).rename("val_dev")
        y_prb = (ratio_use[mask] / med) - 1.0

        try:
            prb_model = sm.OLS(y_prb, sm.add_constant(val_dev)).fit()
            prb = float(prb_model.params["val_dev"])
        except Exception:
            prb = np.nan
    ...
    def handle(self, *args, **options):
        ...
        if not options["dry_run"] and run_stats_list:
            ...
            generate_and_save_methodology_for_run(run_id)
        else:
...
def summarize_value_drivers_from_prb(prb_drivers, top_k=5):
    """
    Take your existing PRB_drivers list and turn it into:
      - value_driver_groups: [{group, importance}]
      - value_drivers:       [{predictor, group, importance, direction}]

    importance is based on PRB 'score', normalized.
    """
    if not prb_drivers:
        return [], []
    ...
    for d in prb_drivers:
        ...
        slope = d.get("slope", 0.0) or 0.0
        if not pred:
            continue
        entry = by_pred.setdefault(
            pred,
            {"score": 0.0, "slope": slope},
        )
        entry["score"] += float(score)

    # Top K predictors by score
    sorted_preds = sorted(by_pred.items(), key=lambda kv: kv[1]["score"], reverse=True)
    top_preds = sorted_preds[:top_k]
    total_score_top = sum(v["score"] for _, v in top_preds) or 1.0

    def find_group(pred_name: str) -> str:
        for g, vars_set in DRIVER_GROUPS.items():
            if pred_name in vars_set:
                return g
        return "Other"

    value_drivers = []
    for pred, info in top_preds:
        group_name = find_group(pred)
        norm_score = info["score"] / total_score_top
        direction = "up" if info["slope"] < 0 else "down"  # slope<0 → higher feature lowers ratio (under-assessed)
        value_drivers.append(
            {
                "predictor": pred,
                "group": group_name,
                "importance": round(norm_score, 3),
                "direction": direction,
            }
        )

    # Group-level importance (using all predictors)
    group_scores = {}
    for pred, info in by_pred.items():
        g = find_group(pred)
        group_scores[g] = group_scores.get(g, 0.0) + info["score"]

    total_group = sum(group_scores.values()) or 1.0
    value_driver_groups = [
        {
            "group": g,
            "importance": round(s / total_group, 3),
        }
        for g, s in sorted(group_scores.items(), key=lambda kv: kv[1], reverse=True)
    ]

    return value_driver_groups, value_drivers




def generate_model_methodology_page_from_stats(stats: List[dict[str, Any]]) -> ModelMethodologyPage:
    """
    Call the OpenAI Responses API and return a validated ModelMethodologyPage instance.
    `stats` should be the list/dict you already store on AdjustmentRunSummary.stats.
    """

    # Convert stats to a JSON blob for the model
    stats_json = json.dumps(stats, indent=2)

    response = openai_client.responses.parse(
        model="gpt-5",  # or "gpt-5.1-mini" if you want cheaper runs
        input=f"{ai_summary_prompt()}\n\nHere is the current stats JSON:\n```json\n{stats_json}\n```",
        text_format=ModelMethodologyPage,  # <-- key line: let SDK parse into Pydantic
        reasoning={"effort": "high"},
    )

    # This is already a Pydantic instance
    page: ModelMethodologyPage = response.output_parsed
    return page


def generate_and_save_methodology_for_run(run_id: str) -> ModelMethodologyPage:
    """
    Fetch AdjustmentRunSummary by run_id, generate methodology page from its stats,
    save it into .content, and return the Pydantic object.
    """
    run = AdjustmentRunSummary.objects.get(run_id=run_id)

    # `run.stats` is whatever you persisted from regression.py (list[dict], etc.)
    page = generate_model_methodology_page_from_stats(run.stats)

    # Store JSON-serializable content back onto the model
    run.content = page.model_dump(mode="json")
    run.save(update_fields=["content"])

    return page

def ai_summary_prompt():
    prompt = """
        # Detailed Prompt for Real Estate Valuation Model Analysis Report

        You are an expert real estate data analyst and technical writer tasked with creating comprehensive, accessible explanations of residential property valuation models. You will analyze JSON data containing statistical modeling results for different geographic markets and property value tiers, then generate clear, professional website copy that helps property owners, assessors, and stakeholders understand how their properties are valued.

        ## Your Task

        Analyze the provided real estate valuation model data and create detailed, reader-friendly explanations for each market segment. Your writing should translate complex statistical concepts into practical insights that help readers understand:

        1. **What the model does** - How properties in their area are valued
        2. **Model performance** - How accurate and reliable the valuations are
        3. **What drives value** - Which property characteristics most influence assessed values
        4. **Price calibration** - How the model handles properties at different price points

        ## Input Data Structure

        You will receive JSON data with the following key fields for each market segment:

        - **label**: Market group and value tier (e.g., "ANACORTES__T1_LOW")
        - **market_group**: Geographic area (e.g., "ANACORTES", "MOUNT_VERNON")
        - **value_tier**: Property value segment (T1_LOW, T2_MID, T3_HIGH, or ALL)
        - **n**: Number of properties in the model
        - **r2**: R-squared value (model fit, 0-1 scale)
        - **adj_r2**: Adjusted R-squared
        - **COD**: Coefficient of Dispersion (uniformity measure)
        - **PRB**: Price-Related Bias (-1 to 1, measures systematic bias)
        - **PRD**: Price-Related Differential (ratio measure of bias)
        - **median_ratio**: Median assessment ratio
        - **price_min/price_max**: Value range for this segment
        - **variables**: List of predictor variables used in the model
        - **value_drivers**: Top 5 most important predictors with importance scores
        - **value_driver_groups**: Aggregated importance by category (Size & layout, Quality & condition, Time & market cycle, etc.)
        - **PRB_drivers**: Variables contributing to price-related bias
        - **calib_bands**: 10 price bands with calibration factors and median ratios

        ## Writing Guidelines

        ### Tone and Style
        - Professional but conversational - write like a knowledgeable advisor explaining to a homeowner
        - Clear and accessible - avoid jargon, or explain technical terms when necessary
        - Specific and data-driven - use actual numbers from the data
        - Balanced - acknowledge both strengths and limitations
        - Helpful and actionable - focus on what the information means for property owners

        ### Content Requirements

        For each market segment, create sections covering:

        #### 1. **Market Overview**
        - Identify the geographic area and value tier clearly
        - State the number of properties analyzed
        - Describe the price range covered
        - Provide context about what this segment represents (e.g., "entry-level homes," "mid-market properties," "luxury segment")

        #### 2. **Model Performance & Accuracy**
        - Explain the R² value in practical terms (e.g., "The model explains 57% of the variation in property values")
        - Translate COD into uniformity assessment (excellent <5, good 5-10, fair 10-15, poor >15)
        - Interpret the median ratio (how close assessed values are to market values on average)
        - Discuss PRB and PRD to explain if the model treats low and high-value properties fairly
        - Give an overall accuracy assessment and note any areas of concern

        #### 3. **What Drives Property Values**
        - Present the top 5 value drivers with their importance percentages
        - Explain what each predictor means in plain language
        - Describe the direction of influence (increases or decreases value)
        - Summarize the grouped driver categories and their combined importance
        - Provide practical insights about what matters most in this market

        #### 4. **Price Calibration & Bias**
        - Explain how the model performs across different price ranges
        - Highlight any systematic biases from PRB_drivers
        - Note which price bands show the best/worst calibration
        - Explain the calibration factors and what they mean for property owners
        - Address whether the model is fair across the price spectrum

        #### 5. **Technical Variables Explained**
        - When referencing variables, provide clear definitions:
        - **log_area**: Property square footage (logarithmic transformation for better modeling)
        - **log_age**: Property age in years
        - **area_quality**: Interaction between size and quality grade
        - **land_share**: Proportion of total value attributed to land vs. improvements
        - **t, t_sq**: Time trend variables capturing market cycles
        - **log_total_mv**: Total market value from assessor records
        - And so on for other variables

        ### Interpretation Standards

        **R² (Model Fit):**
        - 0.40-0.55: Moderate fit, captures major value drivers
        - 0.55-0.70: Good fit, reliable for most properties
        - 0.70-0.85: Strong fit, high confidence in predictions
        - >0.85: Excellent fit, very precise modeling

        **COD (Uniformity):**
        - <5.0: Excellent uniformity
        - 5.0-10.0: Good uniformity
        - 10.0-15.0: Fair uniformity, some inconsistency
        - >15.0: Poor uniformity, significant variation

        **PRB (Price-Related Bias):**
        - -0.05 to +0.05: Minimal bias, fair across price ranges
        - -0.10 to -0.05 or +0.05 to +0.10: Slight bias, monitor
        - Beyond ±0.10: Significant bias requiring attention

        **Median Ratio:**
        - 0.95-1.05: Excellent calibration to market
        - 0.90-0.95 or 1.05-1.10: Good calibration
        - Outside 0.90-1.10: Systematic over/under-valuation

        **Calibration Factors:**
        - Close to 1.0: Well-calibrated price band
        - >1.05: Model tends to under-predict (assessments too low)
        - <0.95: Model tends to over-predict (assessments too high)

        ### Specific Writing Instructions

        1. **Start with the big picture** - What market and value tier is this? How many homes? What price range?

        2. **Lead with accuracy** - Property owners care most about whether valuations are fair and accurate

        3. **Explain the "why"** - Don't just list value drivers, explain what they mean and why they matter

        4. **Be honest about limitations** - If COD is high or PRB shows bias, acknowledge it and explain what's being done

        5. **Use comparisons** - Help readers understand by comparing to standards or other segments

        6. **Include practical implications** - What does this mean for a homeowner in this market?

        7. **Break down complexity** - Use subheadings, bullet points, and clear paragraphs to organize information

        8. **Quantify everything** - Use specific percentages, ratios, and numbers from the data

        9. **Translate statistics** - Turn "r2: 0.5672" into "the model explains about 57% of value variation"

        10. **End with context** - Summarize the overall assessment quality for this segment

        ### Example Explanatory Phrases

        - "In practical terms, this means..."
        - "For a typical homeowner in this segment..."
        - "This suggests that properties with [feature] tend to..."
        - "The model performs particularly well/struggles when..."
        - "Compared to industry standards..."
        - "This level of accuracy indicates..."
        - "Properties in this price range show..."

        ## Output Goal

        Create comprehensive, well-organized website copy that:
        - Builds confidence in the valuation process
        - Educates readers about how their properties are assessed
        - Provides transparency about model performance
        - Helps property owners understand what drives value in their market
        - Acknowledges both strengths and areas for improvement
        - Uses accessible language without oversimplifying

        Your analysis should be thorough enough that a property owner could read it and understand: (1) how accurate their assessment likely is, (2) what factors most influenced their property's value, and (3) whether the model treats properties fairly across different price points.
