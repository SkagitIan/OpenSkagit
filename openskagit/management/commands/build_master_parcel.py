...
from openskagit.models import AssessmentRoll, Assessor, Improvements, MasterParcel
...
EXACT_CODE_CATEGORY = {
    ...
    "GARFIN": "main_structure", "LB-LOFT": "outbuilding", "LOFT": "outbuilding",
...
def resolve_roll_year(df: pd.DataFrame) -> Optional[int]:
    for col in ["tax_year", "appraisal_year"]:
...
def persist_raw_records(model, df: pd.DataFrame, field_map: dict, roll_year: Optional[int]):
    roll = None
    if roll_year is not None:
        roll, _ = AssessmentRoll.objects.get_or_create(year=roll_year)

    if roll is None:
        model.objects.filter(roll__isnull=True).delete()
    else:
        model.objects.filter(roll=roll).delete()

    records = []
    for row in df.itertuples(index=False):
        data = {}
        for source_col, target_col in field_map.items():
            if not hasattr(row, source_col):
                continue
            value = coerce_raw_value(getattr(row, source_col))
            if target_col == "parcel_number":
                value = normalize_parcel(value)
            data[target_col] = value
        if data:
            records.append(model(roll=roll, **data))
...
def load_and_clean_improvements():
    ...
    return raw_impr_df, impr_df, rollup_df
...
def load_and_clean_assessor_and_merge(
    rollup_df: pd.DataFrame,
    raw_impr_df: pd.DataFrame,
) -> pd.DataFrame:
    ...
    raw_assessor_df = df.copy()
    if "sale_date" in raw_assessor_df.columns:
        raw_assessor_df["sale_date"] = pd.to_datetime(
            raw_assessor_df["sale_date"], errors="coerce"
        )
    roll_year = resolve_roll_year(raw_assessor_df)

    assessor_field_map = {
        col: col for col in raw_assessor_df.columns if col in {
            "parcel_number",
            "address",
            "owner_name",
            "owner_add_1",
    ...
    if "parcelnumber" in raw_assessor_df.columns:
        assessor_field_map["parcelnumber"] = "parcel_number"

    improvement_field_map = {
        ...
        "imprv_det_type_cd": "improvement_detail_type_code",
        "imprv_det_class_cd": "improvement_detail_class_code",
        "imprv_det_meth_cd": "improvement_detail_method_code",
        "condition_cd": "condition_code",
        "calc_area": "calculated_area",
        "unit_price": "unit_price",
        "depreciation_pct": "depreciation_pct",
        "imprv_det_value": "improvement_detail_value",
    ...
    persist_raw_records(Assessor, raw_assessor_df, assessor_field_map, roll_year)
    if raw_impr_df is not None:
        persist_raw_records(Improvements, raw_impr_df, improvement_field_map, roll_year)
    ...
    if "living_area" in master.columns:
        master["final_living_area"] = master["living_area"]
    else:
    ...
    area_candidates = [
        col for col in ["final_living_area"] if col in master.columns
    ]
    if area_candidates:
        master["_has_area"] = (
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        raw_impr_df, impr_df, rollup_df = load_and_clean_improvements()

        # 2) Assessor: clean + merge with rollup
        master_df = load_and_clean_assessor_and_merge(rollup_df, raw_impr_df)
